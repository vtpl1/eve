...
from .fieldconfig import (
    ...
    ColumnFieldConfig,
    ColumnPropertyFieldConfig,
    HybridPropertyFieldConfig,
...
class ResourceConfig(object):
    ...
    def render(self, date_created, last_updated, etag, related_resource_configs={}):
        ...
        return {
            ...
            "schema": self._render_schema(field_configs, related_resource_configs),
    ...
    def _deduce_id_field(self):
        ...
        if len(pk_columns) == 1:
            ...
            raise ConfigException(
                ...
                "`ResourceConfig({model}, id_field=...)`".format(
    ...
    def item_lookup_field(self, item_lookup_field):
        if item_lookup_field != self.id_field:
            ...
            if not column.unique:
                raise ConfigException(
                    "{model}.{item_lookup_field} is not unique.".format(
                        model=self.model.__name__, item_lookup_field=item_lookup_field
                    )
                )
    ...
    def _get_column(self, column_name):
        try:
            ...
            raise ConfigException(
                "{model}.{column_name} does not exist.".format(
                    model=self.model.__name__, column_name=column_name
                )
            )
    ...
    def _create_field_configs(self):
        ...
        proxied_relationships = set(
            [p.proxied_relationship for p in association_proxies.values()]
        )
        relationships = {
    ...
    def _get_relationship_fields(self, proxied_relationships):
        return (
            f.key
            for f in self._mapper.relationships
    ...
    def _get_column_fields(self):
        ...
        return (
            ...
            and (f.key == self._id_field or len(f.expression.foreign_keys) == 0)
        )
    ...
    def _render_schema(self, field_configs, related_resource_configs):
        schema = {
            k: v.render(related_resource_configs) for k, v in field_configs.items()
        }
    ...
    def _render_datasource(self, field_configs, etag):
        ...
        return {"source": self.model.__name__, "projection": projection}
